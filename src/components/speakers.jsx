...
const Speakers = () => {
    ...
    const Speaker = ({ name, position, image, linkedin, aos }) => {
        return (<div className="flex flex-col justify-center items-center my-12" data-aos={aos}>
            <a className={`relative w-32 h-32 md:w-44 md:h-44 mx-12 p-2 bg-black outline outline-2 outline-nav-links-unselected ring-4 rounded-full flex justify-center cursor-pointer items-center transform hover:scale-105 hover:outline-primary transition duration-300`}
            ...
            <div className="flex flex-wrap justify-center items-center">
                {
                    speakerList.map((speaker, index) => {
                        return <Speaker key={speaker.name} name={speaker.name} position={speaker.position} image={speaker.image} linkedin={speaker.linkedin} aos={index === 0 ? 'fade-right' : 'fade-left'} />
                    })
                }
            </div>
        </div>
