        ...
        <>
            <div className="flex flex-col justify-between p-10 bg-black md:flex-row md:p-12" id="footer">
                ...
                <div className="flex flex-col items-center justify-center w-full mt-6 md:justify-start md:items-center">
                    ...
                    <a className="mb-8 text-nav-links-unselected hover:text-primary transition duration-300" href="mailto:sllitfoss@gmail.com" target="_blank">sllitfoss@gmail.com</a>
                    <div className="flex justify-start mb-8">
                        {socialLinks.map(link => (
                            <a href={link.url} target="_blank"><img src={`/assets/social/${link.icon}.svg`} className="w-5 h-5 mx-2 fill-current hover:brightness-200 transition duration-300"></img></a>
                        ))}
                    </div>
                    <h1 className="mb-4 text-2xl font-semibold text-white">Visit Us On</h1>
                    <a className="mb-2 text-nav-links-unselected hover:text-primary transition duration-300" href="https://sliitfoss.org" target="_blank">sllitfoss.org</a>
                </div>
                <div className="flex flex-col items-start justify-end w-full mt-6 md:justify-start md:items-end md:pr-24">
                    ...
                    {usefulLinks.map(link => (
                        <a className="mb-3 text-nav-links-unselected hover:text-primary transition duration-300" href={link.url} target="_blank">{link.name}</a>
                    ))}
                </div>
            </div>
            <Divider/>
