...
export function Social() {
  ...
  return (
    <div
      className={`fixed right-[2em] bottom-[6em] text-white z-[49]`}
      style={{ bottom: !pushBottom ? '2.9rem' : '6em' }}>
      <button
        onClick={() => {
        ...
        <div className="w-0 h-0 overflow-hidden whitespace-nowrap group-hover:ml-5 group-hover:h-auto group-hover:w-auto transition-all">
          Join on Whatsapp
        </div>
...
export function WhatsappModal({ showModal, toggleShow }) {
  ...
  return (
    <Portal>
      <div
        className={`fixed w-screen h-screen top-0 left-0 backdrop-blur-2xl z-[99999] font-inter transition-all duration-300 ${
          showModal ? 'opacity-100' : 'opacity-0 pointer-events-none'
        }`}>
        <button onClick={_onClose} className="text-white absolute top-0 p-2 right-0">
          <IoIosClose className="h-14 w-14 text-white cursor-pointer" />
        </button>
        <div className="flex flex-col justify-center items-center w-full h-full text-white">
          <div className="bold text-xl mb-10 font-semibold text-center">
          ...
          <div className="mt-4">
            ...
            <a
              href="https://chat.whatsapp.com/LYR1sVncCWX5ABRvwM8ilu"
