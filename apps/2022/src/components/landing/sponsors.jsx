  ...
  return (
    <>
      <div className="flex flex-col justify-center items-center bg-black p-4 pt-[80px]" id="sponsors">
        ...
        <span className="text-xl text-white font-semibold my-16 mb-12" data-aos="fade-right">
          Gold
        </span>
          ...
          {['Star', 'Rectangle', 'Ellipse'].map((logo, index) => {
            return (
              <Sponsor
                key={logo}
                logo={logo}
                classes="w-24 h-24 md:w-[8rem] md:h-[8rem] p-10 mx-6"
                aos={index % 2 === 0 ? 'fade-down' : 'fade-up'}
              />
            );
          })}
        </div>
        <span className="text-xl text-white font-semibold my-16 mb-12" data-aos="fade-left">
          Silver
        </span>
          ...
          {['Triangle', 'Triangle', 'Triangle', 'Triangle', 'Triangle', 'Triangle', 'Triangle'].map((logo, index) => {
            return (
              <Sponsor
                key={index}
                logo={logo}
                classes="w-16 h-16 md:w-[6rem] md:h-[6rem] p-5 mx-6"
                aos={index % 2 === 0 ? 'fade-down' : 'fade-up'}
              />
            );
          })}
