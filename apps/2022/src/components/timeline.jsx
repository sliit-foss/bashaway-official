...
const Timeline = () => {
  ...
  return (
    <div className="flex flex-col bg-black h-full justify-center items-center pt-24 px-6" id="timeline">
      <div className="bg-[#7928CA]" />
      <Title title="Timeline" aos="fade-down" />
      {timelineList.map((list) => {
        return (
          <div key={list.title} className="flex flex-col justify-center items-center">
            <span
              className={`${
                list.isSmallText ? 'text-[20px]' : 'text-[32px]'
              } mt-[30px] text-white text-center font-semibold leading-6"`}
              data-aos="fade-down">
              {list.title}
            </span>
            {list.show !== false && (
              <div className="mt-5" data-aos="fade-down">
                <div
                  className={`relative flex justify-center h-[80px] w-0.5`}
                  style={{ backgroundImage: `linear-gradient(to top, ${list.gradientColor1}, #ffffff00)` }}>
                  <div
                    className="absolute w-6 h-6 rounded-full bottom-0"
                    style={{
                      backgroundImage: `linear-gradient(to right, ${list.gradientColor1}, ${list.gradientColor2})`
                    }}></div>
                </div>
              </div>
            )}
